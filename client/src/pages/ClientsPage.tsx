    ...
    <main className="pt-20">
      ...
      <div className="relative min-h-[360px] flex items-center overflow-hidden">
        <div className="absolute inset-0">
          <img
            src="https://images.unsplash.com/photo-1521737852567-6949f3f9f2b5?w=1600&auto=format&fit=crop"
            alt="Clients"
      ...
      <section className="py-20 bg-gray-50">
        <div className="container mx-auto px-6">
          <p className="text-center text-gray-400 text-sm mb-14">Selected clients include International and Freightliner</p>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7 mb-16">
            {clients.map((c) => (
              <div
                key={c.name}
                className="bg-white rounded-2xl border border-gray-200 p-9 relative overflow-hidden hover:-translate-y-1 hover:shadow-xl transition-all duration-300"
              >
                {/* Top accent line */}
                <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[#0055A5] to-[#00A896]" />

                <div className="text-2xl font-black text-[#0B1628] tracking-widest mb-1" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
                  {c.name.replace(c.nameAccent, "")}
                  <span className="text-[#00A896]">{c.nameAccent}</span>
                </div>

                <span className="inline-block bg-[#00A896]/10 text-[#00A896] text-xs font-bold tracking-widest uppercase px-3 py-1 rounded-full mb-5">
                  {c.tag}
                </span>
                ...
                <div className="space-y-2.5">
                  {c.contact && (
                    <div className="flex gap-3 text-sm">
                      <span className="text-xs font-bold uppercase tracking-wide text-[#0B1628] min-w-[60px]">Contact</span>
                      <span className="text-gray-500">{c.contact}</span>
                    </div>
                  )}
                  {c.role && (
                    <div className="flex gap-3 text-sm">
                      <span className="text-xs font-bold uppercase tracking-wide text-[#0B1628] min-w-[60px]">Role</span>
                      <span className="text-gray-500">{c.role}</span>
                    </div>
                  )}
                  {c.tel && (
                    <div className="flex gap-3 text-sm">
                      <span className="text-xs font-bold uppercase tracking-wide text-[#0B1628] min-w-[60px]">Tel</span>
                      <span className="text-gray-500">{c.tel}</span>
                    </div>
                  )}
                  {c.email && (
                    <div className="flex gap-3 text-sm">
                      <span className="text-xs font-bold uppercase tracking-wide text-[#0B1628] min-w-[60px]">Email</span>
                      <a href={`mailto:${c.email}`} className="text-[#00A896] hover:underline">{c.email}</a>
                    </div>
                  )}
                </div>

                <div className="mt-5 pt-5 border-t border-gray-100 bg-gray-50 -mx-9 -mb-9 px-9 pb-7 rounded-b-2xl">
          ...
          <div className="max-w-2xl mx-auto bg-white rounded-2xl border border-gray-200 p-12 text-center">
            <h3 className="text-4xl font-black text-[#0B1628] mb-4 tracking-wide" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
            ...
            <button
              ...
              className="bg-[#F4A022] hover:bg-[#e8940f] text-[#0B1628] font-bold px-10 py-4 rounded-lg text-sm uppercase tracking-wide transition-all hover:-translate-y-1 hover:shadow-lg"
            >
              Get in Touch
            </button>
          </div>
